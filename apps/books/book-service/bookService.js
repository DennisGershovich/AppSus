...
function getById(bookId) {
    const book = books_list.find(book => bookId === book.id)
    return Promise.resolve(book)
}

function remove(bookId) {
    books_list = books_list.filter(book => book.id !== bookId)
    return Promise.resolve()
}


function getNextBook (bookId) {
    const bookIdx = books_list.findIndex(book => bookId === book.id)
    const nextBookIdx = (bookIdx + 1 === books_list.length) ? 0 : bookIdx + 1
    return books_list[nextBookIdx].id
}